...
class CommitteeController extends Controller
{
    ...
    public function project(){
        $rules = Rule::with(['actives', 'duas', 'duls', 'countries', 'articles', 'classifications', 'baskets', 'status'])->where('status_id', '=',1)->get();
        return view('rule.ruleIndex', compact('rules'));
    }

    public function examining(){
        $rules = Rule::with(['actives', 'duas', 'duls', 'countries', 'articles', 'classifications', 'baskets', 'status'])->where('status_id', '=',2)->get();
        return view('rule.ruleIndex', compact('rules'));
    }

    public function approved(){
        $rules = Rule::with(['actives', 'duas', 'duls', 'countries', 'articles', 'classifications', 'baskets', 'status'])->where('status_id', '=',3)->get();
        return view('rule.ruleIndex', compact('rules'));
    }
